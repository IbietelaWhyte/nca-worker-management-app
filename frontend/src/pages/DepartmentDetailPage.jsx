...
export default function DepartmentDetailPage() {
  ...
  const { department, loading, error, addMember, removeMember, assignHod } = useDepartmentDetail(id)
  ...
  const handleRemoveMember = async (worker) => {
    if (!confirm(`Remove ${worker.first_name} ${worker.last_name} from this department?`)) return
      ...
      <div className="space-y-4">
        ...
        {(!department.workers || department.workers.length === 0) ? (
          <div className="flex flex-col items-center justify-center h-40 border rounded-lg">
            ...
            <Button variant="outline" size="sm" className="mt-3" onClick={() => setAddMemberOpen(true)}>
              Add first member
            </Button>
          </div>
        ) : (
