...
- Tailwind is pinned at **3.4.19**, but several `ui/` primitives came from the shadcn radix-nova registry, which targets v4. Around 60 v4-only utilities (`size-(--cell-size)`, `--spacing()`, `data-open:`, `outline-hidden`) are silently dropped at build — mostly in `calendar.jsx`. The three that affected visible styling (`ring-3`, `rounded-4xl`, `size-3!`) are fixed; **prefer v3 syntax in `ui/` until someone migrates to v4**, which would resolve the rest at once.
