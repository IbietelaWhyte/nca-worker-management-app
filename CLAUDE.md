...
- **Route declaration order is load-bearing.** A literal segment must be declared before the `/{uuid}` route that would otherwise swallow it — `/departments/{id}/workers/import` before `/{department_id}/workers/{worker_id}`, `/leave/current` before `/leave/{leave_id}`, `/availability/config` and `/availability/link/{token}` before `/availability/{availability_id}`. FastAPI matches in declaration order, so the symptom is a 422 complaining that "current" is not a valid UUID.
