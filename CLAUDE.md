...
Some Sundays are bigger than others, and to a least-served-first sort one Sunday looks exactly like the next — so the same few names land on every big service. `special_services` (`20260918093000_special_services.sql`) is **one table with a `kind`**, matching `availability` and `availability_prompts` rather than splitting recurring rules from named one-offs: they are the same thing to every reader and differ only in how the date is arrived at.

- `(day_of_week, week_of_month)` with **-1 meaning last** covers first/second/third/fourth/last: 35 expressible rules and nothing to parse. Fortnightly needs an anchor date and is deliberately inexpressible. A fifth occurrence resolving to no date in a short month is correct, not an error.
- **Admin-only, because the rules are not scoped to a department** — every team's rota on a matching date is planned as special. A head moving "first Sunday" would change every other department's dates. `AdminUser` on the writes, mirrored by RLS.
- **`service/special_services/rules.py` is pure**, a sibling of `planner.py` for the same reason. **The whole 0=Sunday vs Python's 0=Monday conversion lives in `_to_python_weekday` there** — CLAUDE.md already flags that mismatch and this is its third site. `test_special_service_rules.py` asserts against known real dates (2 August 2026 is a Sunday), so a self-consistent bug cannot pass.
- **The schedule is stamped, never re-derived.** `schedules.special_service_name` is a snapshot and `special_service_id` is `ON DELETE SET NULL`, so a rota keeps saying "Jesus is Lord Service" after a rename *and* after the rule is deleted. Re-deriving would let an admin editing "first Sunday" to "second Sunday" retroactively zero everyone's tally — history changing under them. `special_service_name is not null` is the single "was this special" test.
- **Every date has a *kind*, and fairness is counted and spaced within it.** The kind is `special_service_name` or `ORDINARY` (`""`), so the ordinary Sundays are one stream and each named service its own. `special_count` and `last_served` are keyed `(worker, kind)`. One tally shared across all specials was not enough: it reads six Thanksgivings and no Christmas as level with the reverse, which is exactly how somebody ends up on every Thanksgiving for a year. A special pick increments **every** counter — a big service is still a turn. Unwindowed for the same reason as before, and still free, the history fetch already embedding the schedule.
- **`_fairness_key` is six terms: `(streak, load, total_count, last_served, last_assigned, id)`.** `load` is `month_count` on an ordinary date and the count for that named service on a special one.
- **`streak` is the spacing term, and the only one about *when* rather than *how many*.** It counts consecutive turns at that kind and leads the sort, so whoever sat the last one out goes first and a shorter run outranks a longer one. Counts alone are equally happy with four Thanksgivings in a row then three months off; a volunteer needs to attend the big services, not only work them. It is a **sort, not a filter** — where fewer people are fresh than the group needs, the shortest runs come straight back up and the date is still staffed. Where a repeat is forced (four slots from seven people), this is what makes the repeat rotate instead of landing on one person. `_streaks_from` seeds it from history, without which every month would start believing nobody had served recently and the spacing would reset each January.
- **A missed service ends a run, however it was missed.** `_plan_group` clears the streak for every group member it did not pick, including anyone dropped as unavailable — otherwise a fortnight of leave leaves somebody stuck behind a run they never had.
- **`total_count` is what survives a month boundary.** `month_count` restarts each month, so a month whose slots do not divide evenly forgets its remainder every time; a year of remainders is whole services of difference. It is the tie-break under both load terms. This replaced the old "other tally as tie-break", which was structurally dead: a first-Sunday rule always resolves to the earliest date in its month, so `month_count` was 0 for everybody at the moment it was consulted — and first-Sunday is the commonest rule there is.
- **Eligibility is untouched.** A special date is staffed from exactly the same roster; the tallies change the order, never who is in the running.
- **A band that is an exact multiple of the roster partitions it, and that is correct.** Three slots filled from six people splits the group into two trios that alternate forever, because on every date the three who did not serve last time have both the shorter run and the lower count. Mixing them would mean scheduling somebody two weeks running while a colleague rests. Widen the roster or the band if the split is unwanted — it is not a planner bug.
- `commit_monthly_schedule` **re-resolves server-side**, so nothing the client sends can mislabel a date — which is why `DateSelection` gained no field for it.
...
- `/special-services` is **admin-only and says so itself**: `ProtectedRoute` has no role prop, so the page renders an "administrators only" panel rather than bouncing a head who follows a link. The sidebar's `adminOnly` flag sits beside the existing `manageOnly`. Two card-list sections and no `<Table>` — three fields per row is legible at every width, so this page skips the mobile split deliberately. A rule is entered as a **sentence with two selects** ("On the [first] [Sunday] of every month"), never a cron editor, and `describeSpecialService` in `src/lib/specialServices.js` renders the same sentence back.
- A special date is marked in five places, all gold (`Badge variant="warning"`) and **never red**, since a date can be both special and understaffed: `MonthPreviewTable` (highest value — where a head decides which dates to keep), `MonthCalendar` (a gold ring on the day chip, so it survives the today chip taking the fill), `SchedulesPage`, `ScheduleDetailPage`, and `RotaExportDialog`, which needs its own fixed `SPECIAL_BG = '#ECCE68'` — the saturated brand gold, where `HIGHLIGHT` is a pale tint of it, so the two read apart on one page. Four of the five need **no extra request**: `special_service_name` rides on `ScheduleResponse`.
...
Core tables: `workers`, `worker_app_roles`, `worker_leave`, `departments`, `department_roles`, `worker_departments` (junction, carries `department_role_id` + `subteam_id`), `subteams`, `availability`, `schedules`, `schedule_assignments`, `assignment_reminder_sends`, `special_services`, `department_assistant_hods`, `confirmation_tokens`. RLS helpers defined in SQL: `current_worker_id()`, `has_app_role()`, `is_hod(dept_id)`.
