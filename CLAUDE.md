...
- **The planner's `_fairness_key` swaps which tally leads.** On a special date `special_count` leads and `month_count` becomes the tie-break; on an ordinary date the reverse. The other tally is the tie-break rather than discarded, so a special date cannot ignore the month's balance. A special pick increments **both** — a big service is still a turn. `special_count` is **all-time, not windowed**: specials fall perhaps fifteen times a year, so a within-month count would be 0 for everybody and order nothing. It costs no extra query, the history fetch already embedding the schedule.
- **Eligibility is untouched.** A special date is staffed from exactly the same roster; the tally changes the order, never who is in the running.
