...
class ScheduleService:
    def __init__(
        ...
        special_service_service: SpecialServiceService,
    ) -> None:
        ...
        # Which dates are bigger than an ordinary service. Resolved once per planning run and
        # passed into the context, so a preview and the commit that follows it cannot
        # disagree about which Sundays were special.
        self.special_service_service = special_service_service
    ...
    def generate_schedule(self, data: ScheduleCreate, created_by: str) -> ScheduleResponse | None:
        ...
        # One date, but resolved through the same call the month path uses so a lone
        # Sunday generated by hand is judged special on exactly the same rules.
        special_dates = self.special_service_service.get_special_dates(scheduled_date, scheduled_date)
        ...
        schedule_data = {
            ...
            # Snapshot, not a link: the name has to survive a rename and the rule being
            # deleted, because it is what the fairness tally counts and the export prints.
            q.Columns.SPECIAL_SERVICE_NAME: special_dates.get(scheduled_date),
    ...
    def preview_monthly_schedule(self, data: MonthlySchedulePreviewRequest) -> MonthlySchedulePreview:
        ...
        # Resolved once for the whole month and reused for both the plan and the badges, so
        # the preview cannot show a date as special that the planner rotated as ordinary.
        special_dates = self.special_service_service.get_special_dates(month_start, month_end)
        ...
        return MonthlySchedulePreview(
            ...
            dates=[
                DatePlan(
                    ...
                    special_service_name=special_dates.get(plan.scheduled_date),
                    groups=[
                        PlannedGroup(
    ...
    def commit_monthly_schedule(self, data: MonthlyScheduleCommitRequest, created_by: str) -> MonthlyScheduleResult:
        ...
        # Re-resolved server-side rather than taken from the request. Nothing the client
        # sends can mislabel a date as special or hide that it is, which is why
        # DateSelection gained no field for it — the rota's own record of what it served
        # must come from the rules, not from whatever the browser last saw.
        special_dates = self.special_service_service.get_special_dates(requested[0], requested[-1])
        ...
        schedule_rows = [
            {
                ...
                q.Columns.SPECIAL_SERVICE_NAME: special_dates.get(selection.scheduled_date),
                q.Columns.CREATED_BY: str(created_by_user.id),
            }
    ...
    def _build_plan_context(
        ...
        month_start: date,
        month_end: date,
        special_dates: dict[date, str] | None = None,
    ) -> PlanContext:
        ...
        `special_dates` is passed in rather than resolved here so the caller resolves the
        window once and reuses it for the badges on screen as well as the plan.
        """
        plan_groups = [
